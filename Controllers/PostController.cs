...
namespace FNF_PROJ.Controllers
{
    ...
    public class PostsController : ControllerBase
    {
        ...
        {
            var dto = await _postService.GetPostByIdAsync(id);
        ...
        [HttpGet("feed")]
        [Authorize]
        public async Task<IActionResult> GetFeed([FromQuery] int? deptId, [FromQuery] int skip = 0, [FromQuery] int take = 50)
        {
            var list = await _postService.GetAllPostsAsync();
            return Ok(list);
        }

        // IMPORTANT: use [FromForm] because request is multipart/form-data with files
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> CreatePost([FromForm] PostCreateDto dto)
        {
        ...
        [HttpPut("{postId}")]
        [Authorize]
        public async Task<IActionResult> EditPost([FromRoute] int postId, [FromForm] PostCreateDto dto)
        {
            var uid = GetCurrentUserId();
            if (uid == 0) return Unauthorized();

            // get role and dept from claims (adjust claim names as your token uses)
            var role = User.FindFirst(ClaimTypes.Role)?.Value ?? "";
            var deptIdClaim = User.FindFirst("deptId")?.Value;
            var deptId = int.TryParse(deptIdClaim, out var d) ? d : 0;

            var updated = await _postService.EditPostAsync(uid, role, deptId, postId, dto);
            return Ok(updated);
        }

        [HttpDelete("{postId}")]
        [Authorize]
        public async Task<IActionResult> DeletePost([FromRoute] int postId, [FromQuery] string reason = "")
        {
            var uid = GetCurrentUserId();
            if (uid == 0) return Unauthorized();

            // deptId claim to verify manager's dept
            var deptIdClaim = User.FindFirst("deptId")?.Value;
            var deptId = int.TryParse(deptIdClaim, out var d) ? d : 0;

            await _postService.DeletePostAsync(uid, deptId, postId, reason);
            return NoContent();
        }
        ...
        public async Task<IActionResult> Repost([FromBody] RepostDto dto)
        {
            ...
            var created = await _postService.RepostAsync(uid, dto);
            return Ok(created);
        }
    }
}
